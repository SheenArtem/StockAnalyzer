...
FIELD_GLOSSARY = {
    ...
    'bank_reserves': ('銀行存款準備金餘額 WRESBAL', '十億美元', 'RRP 乾涸後成為邊際流動性緩衝;QT/TGA 補庫直接打準備金'),
    'bank_reserves_chg_4w': ('銀行準備金 近4週變化', '十億美元', '負=準備金流失(流動性收緊)'),
    'iorb': ('存款準備金利率 IORB', '%', 'Fed 付給銀行準備金的利率(利率走廊下緣);2021-07 起'),
    'sofr_iorb_spread': ('SOFR 減 IORB 利差', '百分點', '資金壓力在「利差」非「水位」;走闊(SOFR>IORB)=回購市場緊/準備金稀缺早警'),
    'us_real_yield_10y': ('美國10年期實質殖利率 (TIPS)', '%', '估值折現率核心;實質利率上行=壓抑高估值;與 breakeven 併看拆名目利率'),
    'us_real_yield_10y_chg_4w': ('10年實質殖利率 近4週變化', '百分點', '正=實質利率走升(估值逆風)'),
    'us_breakeven_10y': ('美國10年期通膨預期 (breakeven)', '%', '=名目10Y−實質10Y;油價衝擊下尤關鍵'),
    'ig_oas': ('美國投資級公司債信用利差 (IG OAS)', '%', '補信用品質階梯 IG->HY->CCC;IG 走闊=壓力往優質端遷移'),
    'ig_oas_rank': ('投資級債利差 近10年百分位', '0-100', '高=IG 信用壓力升'),
    ...
    'margin_to_mktcap_pct': ('融資餘額佔市值比重', '%', '官方融資金額/上市總市值;越高=散戶槓桿越重(過熱);台積電灌大市值後結構性偏低(近期~0.4%);當代頂部帶 0.43-0.53%(2018-2025 各大頂前),>=0.48 過熱警示(Group B flag,門檻需定期 review)'),
    'margin_mktcap_z_252d': ('融資佔市值比 252日 z-score', 'z', '正值大=融資相對市值偏高(槓桿擴張);負值大=去槓桿'),
    'margin_maintenance_pct': ('大盤融資維持率', '%', '融資擔保品市值(Σ個股融資張x收盤,含ETF)/官方融資金額;主管機關不公布、各家自算(同 M平方/XQ 公式,對 istock 2026-01 快照誤差 0.4%);新倉基準166.7%(融資成數6成),2021-2026 實測多頭 170-210/空頭壓縮至 125-150;<140=追繳壓力區(法定追繳線130;2022-10 谷 123.1/2025-04-09 谷 124.9/2024-08-05 谷 142.4);急跌時為恐慌/斷頭賣壓計, 非頂部指標'),
    'margin_collateral_value': ('融資擔保品總市值', '元', '所有融資股票按收盤 mark-to-market 總市值(含融資 ETF);維持率分子'),
    'total_market_cap': ('上市總市值', '元', 'Sum(個股收盤x發行股數);台股上市大盤規模'),
    ...
    'copper_gold_ratio': ('銅金比 (銅價/金價)', '比', '升=成長預期>避險需求(risk-on,常與殖利率同向);降=轉避險'),
    'copper_gold_chg_4w': ('銅金比 近4週變化', '%', '正=景氣樂觀轉強'),
    'cl_close': ('WTI 原油價格', '美元/桶', '面板有 OVX 油波動,此為油價本身;急升=通膨/地緣風險'),
    'cl_chg_4w': ('WTI 原油 近4週變化', '%', '正=油價走升'),
    ...
    # ---- 領頭羊 / 跨市場領先 (SOX/Nasdaq 位階+相對強弱 + TSM ADR 溢價) ----
    ...
    'sox_close': ('費城半導體指數 SOX 收盤', '點', '台股領頭羊複合體;美盤收於台北深夜,為 TWII 次日開盤的隔夜領先'),
    'sox_chg_1d': ('費城半導體指數 SOX 1日漲跌', '%', '昨晚美盤表現;隔夜訊號'),
    'sox_dist_ma20': ('SOX 對 20 日均線乖離率', '%', '(指數−MA20)/MA20;在 SOX 自身交易日序列計算'),
    'sox_dist_ma50': ('SOX 對 50 日均線乖離率', '%', '中期乖離'),
    'sox_dist_ma200': ('SOX 對 200 日均線乖離率', '%', '長期乖離;過高=偏離長均過遠(均值回歸風險升)'),
    'nasdaq_close': ('那斯達克綜合指數收盤', '點', '美科技股大盤;台股電子權值的隔夜領先'),
    'nasdaq_chg_1d': ('那斯達克 1日漲跌', '%', '昨晚美盤表現;隔夜訊號'),
    'nasdaq_dist_ma20': ('那斯達克 對 20 日均線乖離率', '%', '(指數−MA20)/MA20'),
    'nasdaq_dist_ma50': ('那斯達克 對 50 日均線乖離率', '%', '中期乖離'),
    'nasdaq_dist_ma200': ('那斯達克 對 200 日均線乖離率', '%', '長期乖離'),
    'nasdaq_to_twii_ratio': ('那斯達克 除以 台股加權 比', '比', '絕對值無意義,看下方變化'),
    'nasdaq_rs_chg_4w': ('Nasdaq/TWII 相對強弱 近4週變化', '%', '正=美科技股領先台股;轉負=台股失去外部動能支撐'),

    # ---- 台灣總經/基本面領先指標 (國發會 LEI, 月頻) ----
    'lei_composite': ('台灣景氣領先指標 綜合指數 (國發會)', '', '月頻;綜合 7 分項,領先景氣轉折約 3-6 月'),
    'leading_export_order_idx': ('外銷訂單動向指數', '指數', '>50=擴張,<50=收縮;台股盈餘需求面領先;為動向指數非金額 YoY'),
    'leading_m1b': ('貨幣總計數 M1B', '百萬元', 'M1A+活期儲蓄存款;升=資金寬鬆;絕對值大,看趨勢'),
    'leading_semi_equip_import': ('半導體設備進口值', '', '台灣科技資本支出領先;升=擴產循環'),
}
