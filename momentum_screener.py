...
DEFAULT_CONFIG = {
    ...
    # 流動性門檻 5 億：Momentum/QM 快進快出需要較高流動性；
    # Value 使用 3 億門檻以涵蓋中型價值股。
...
class MomentumScreener:
    ...
    def run(self, market='tw', mode='momentum'):
        ...
        Args:
            ...
            mode: 'momentum' (5-20d) or 'qm' (quality momentum)
    ...
    def _stage1_filter(self, df):
        ...
        if tv_data:
            ...
            # 2026-05-16 修：legacy fallback 補上特別股 / 權證排除（TV 路徑已 tv_clean 過濾，
            # 但 legacy 走 raw df，特別股 2881A 可能漏網）
            self.progress("  Stage 1: TradingView unavailable, using legacy pct filter")
            df = df[df['stock_id'].astype(str).str.match(r'^[1-9]\d{3}$')].copy()
            results = []
    ...
    def _stage2_analyze(self, candidates):
        ...
        if mode == 'qm':
            ...
            # Momentum mode: trigger_score 排序
            # (swing mode + rvol_lowatr 排序/Top20 標記 已移除 2026-06-07 — 依據
            #  「Sharpe 9.50」被 reports/rvol_atr_factor_validation.md 推翻 (污染 panel
            #  h=60 sharpe_proxy; clean h=10/20 combo FAIL)，且管線 4/15 起無排程)
    ...
    def _analyze_single(self, stock_id, market_row):
        ...
        # 8. (removed 2026-06-07) rvol_lowatr 第二層 filter — 依據被
        #    reports/rvol_atr_factor_validation.md 推翻 (ATR% leg = stale-price
        #    artifact + 方向反；combo clean h=10/20 FAIL)
