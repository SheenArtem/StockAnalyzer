...
起因（2026-08-02 實測）：TPEX **舊**端點 `stk_quote_result.php` **完全無視 `d`
參數** —— 請求 115/06/16（6 週前）回的是 115/07/31 的橫斷面，價格一字不差；請求
週六 115/08/01 同樣回 07/31。TWSE MI_INDEX 則正確分辨，非交易日回
stat="很抱歉，沒有符合條件的資料!"。

✅ 那個端點已於同日改為認日期的 `dailyQuotes`（見 `tests/test_tpex_daily_date_aware.py`），
但**本檔的測試照留**：這裡驗的是「payload 自報日期不符就丟掉」這道防線，
它不該因為某個端點修好而拆除 —— 端點會變，下一個壞掉的可能不會有人先發現。
所以下方 `_stale_cross_section` 仍刻意模擬「無視請求日期」的端點行為。
...
def test_thin_proxy_dates_are_dropped():
    ...
    `ohlcv_tw.parquet` **原本**有 11 個真實交易日只存了 33~38% 的橫斷面（官方 MI_INDEX
    證實那些日子有 1,100~1,300 檔成交，是 yfinance 端缺資料，連 2330 都沒有），
    top300 代理在那些天只剩約 100~132 檔而非 294，2016-06-04 因此產生
    |ret_20d| > 30% 的假值。

    ✅ 那 11 天已於 2026-08-02 由 `tools/backfill_panel_gaps.py` 回填，10 天不再觸發
    這道門檻。**本測試用合成 counts，不讀真實 panel，所以不受回填影響** —— 而且該留：
    驗的是「成分不足就要剔除」這條規則，下一批缺資料仍要靠它。
    """
