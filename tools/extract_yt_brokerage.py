...
CHANNEL_TYPE_HINTS = {
    "personal": (
        ...
        "- host_name 寫主持人姓名 (e.g. Lily / 德瑜)\n"
        "- analyst_view / mentions 都是 {analyst_name} 的觀點"
    ),
    "rotating_guest": (
        "影片結構: 此頻道為「{brokerage_name}」旗下節目「{analyst_name}」(單頻道輪換來賓制),"
        "與分析師個人頻道不同。每集通常 1 位固定主持人 + 1 位輪換來賓分析師。\n"
        "- host_name 寫當集**主分析師 (來賓)** 的姓名 (從字幕識別,e.g. 陳唯泰 / 邱志洲 / 鍾國忠 / 應佳臻),"
        "節目固定主持人 (如游豐進) 不寫到 host_name\n"
        "- 字幕無法識別主分析師姓名時 host_name 留空字串\n"
        "- analyst_view 是當集主分析師對台股大盤的看法\n"
        "- mentions 是當集主分析師討論的個股"
    ),
}
...
def extract(vtt_path: Path, brokerage: str, analyst_key: str,
    ...
    channel_type = BROKERAGES[brokerage].get("channel_type", "personal")
    channel_hint = CHANNEL_TYPE_HINTS.get(channel_type, CHANNEL_TYPE_HINTS["personal"]).format(
        brokerage_name=brokerage_name, analyst_name=analyst_name,
    )
