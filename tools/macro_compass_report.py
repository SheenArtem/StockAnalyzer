...
→ 以上結構序列**以面板為準**。但面板結構上拿不到下列兩類，**請務必上網查證補上**（hybrid 重點）：
（1）**校正會落後的快速值**（面板取自排程，盤後/即時值會落後）：VIX（面板取自 FRED，晚約 1 個交易日）、美國密大消費信心（月頻，面板可能落後 1-2 個月，查最新終值+初值）、各主要指數**距近 1 個月高點的回檔幅度%**（面板無此欄：TWII / S&P500 / SOX）。查到 live 值與面板不符時**以 live 為準**，並於第 5 段標註（面板舊值→live 新值＋來源＋日期）。
（2）**未來 1-2 週經濟行事曆與突發催化**（面板完全沒有，是最關鍵的前瞻盲點）：美國 CPI/PPI 最新印出值、FOMC 與各國央行（含日本央行）利率決議的**日期與市場預期**、重大地緣事件、權值股（台積電/輝達/博通等）財報或重大利空。
網路引用的數字/事件**務必附來源與日期**；面板已涵蓋的結構序列**不要**上網重抓。

【已評估、無免費源 / 付費 / IC 否決 — 上網時不必再找這些（已決定不補，非缺口）】
...
{H(5, "資料時效校正與本週前瞻催化（上網查證結果）")}
- **面板時效校正**：列出上網後校正過的面板數值 — 哪個面板值已過時、live 最新值、來源與日期（例：VIX 面板 19.44〔6/11〕→ live 17.68〔6/12〕；密大消費信心面板 49.8〔4月〕→ 最新終值/初值）。沒有需要校正的也明說「面板與 live 一致」。
- **本週前瞻催化（未來 1-2 週）**：日期 ＋ 事件 ＋ 市場預期 ＋ 對台股可能影響（例：6/17 FOMC、最新 CPI 印出、各國央行決議、地緣、權值股財報/利空）。
- 若以上查證**改變了第 1 段風險定調或第 2 段情境機率**，明確說明如何調整。
