...
- **必搜兩類高訊號事件，找到就一定要寫進報告**：①**券商評等/目標價調整**（具名外資/投信 + 目標價數字 + 日期 → chip.rows 分析師列 + bull_bear）②**ETF 成分股納入/剔除**（0050/0056/高股息/主題 ETF，被動資金 catalyst，含生效日 → chip.rows + left_right.catalysts）。系統 [ANALYST_TARGETS] 台股稀疏、ETF 納入無系統 feed，這兩類只能靠 WebSearch。
- **時效防呆**：目標價/評等這類會被反覆修訂的數字，務必找「**最新一次**」並核對發布日期，**舊值已被新值取代不可引用**（同券商先前 140、最新 255 → 只能用 255）；多筆衝突取日期最新那筆。
