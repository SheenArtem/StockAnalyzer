...
| 0.11 | 2026-06-15 | **流動性門檻 10M → 100M TWD + 回測/現狀接縫修復**：(1) 門檻上調三處同步 (screener / trade_ledger / phase2 default `1e7`→`1e8`) — 排除聲寶(1090萬)/統振(1730萬)/欣陸(2770萬)等低流動性小型股, universe 1839→582 檔. Portfolio backtest Sharpe 1.67→1.35 / CAGR 28%→25% (砍 illiquid noise premium 的誠實 tradeoff), 仍勝 TWII (Sharpe 0.997 / CAGR 17.3%). (2) **接縫修復**: `trade_ledger.build_v13_feat` 補 KY/ETF/特別股 hard exclusions (過去漏掉害回測選到 5871 中租-KY 等 production 不選的股); `ledger_append --rebal` 新增 system top-K 進出維護 (平倉跌出榜 + append 新進 + holding_months + 殭屍價 fallback + meta 同步) → 歷史段 (full rebuild ≤2026-05) 之後每月 M15 無縫接真實 PIT cohort, ledger 末端「持有中」= 當前實際持倉 `_active_holdings` (不再各算各的). `whale_picks_view.py` 標接縫 caption + 涵蓋期間. |
...
| Universe | TW 3610 PIT (含已下市) filtered to **~582 liquid stocks** (avg_tv ≥ 100M TWD + Vol ≥ 300 lots) | aa045f6 PIT 避免 survivor bias + 2026-06-15 門檻 10M→100M (v0.11) |
| Liquidity hard filter | **avg_tv_60d ≥ 100M TWD AND latest Vol ≥ 300 lots** | v13 (SPEC §3 C5) + 2026-06-15 上調 100M (v0.11) — 容量/滑價考量 |
